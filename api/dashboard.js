...
export default async function handler(req, res) {
    ...
    let rowsHtml = '';
    if (fetchError) {
        rowsHtml = `<tr><td colspan="6" class="empty-state error-state">⚠️ ${fetchError}</td></tr>`;
    } else if (registrations.length === 0) {
        rowsHtml = `<tr><td colspan="6" class="empty-state">🦌 No signups yet — share the landing page to get started!</td></tr>`;
    } else {
        registrations.forEach((u, i) => {
            ...
            <tr class="data-row">
                ...
                <td><span class="phone-chip">${escapeHtml(u.phone || '—')}</span></td>
